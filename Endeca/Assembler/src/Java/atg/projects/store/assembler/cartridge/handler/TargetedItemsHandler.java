...
/**
 * <p>
 *   TargetedItemsHandler adds a configurable prefix to the cartridge targeter 
 *   component path if needed, resolves the targeter component, targets for a 
 *   set number of items and adds them into TargeterItems.
 * </p>
 * <p>
 *   Properties:
 *   <ul>
 *     <li>
 *       sourceMap - Targeting source map to be used during targeting. 
 *       Represents a mapping between source names used in a targeting operation
 *       and their Nucleus paths. sourceMap is used to obtain a NameResolver
 *       object and passes it to the Targeter, which uses the NameResolver to 
 *       resolve the names of source objects associated with a targeting 
 *       operation. 
 *     </li>
 *     <li>
 *       itemDescriptor - Descriptor of item that should be returned be this 
 *       targeter.
 *     </li>
 *     <li>   
 ...
 * @version $Id: //hosting-blueprint/B2CBlueprint/version/10.2.1/Endeca/Assembler/src/atg/projects/store/assembler/cartridge/handler/TargetedItemsHandler.java#4 $$Change: 804523 $
 * @updated $DateTime: 2013/04/22 06:50:08 $$Author: ckearney $
 */
public class TargetedItemsHandler extends NavigationCartridgeHandler<ContentItem, TargetedItemsContentItem> {
  
  public static final String CLASS_VERSION = "$Id: //hosting-blueprint/B2CBlueprint/version/10.2.1/Endeca/Assembler/src/atg/projects/store/assembler/cartridge/handler/TargetedItemsHandler.java#4 $$Change: 804523 $";
  ...
  /**
   * @param pItemDescriptor - The item descriptor of item that should be used in
   * this targeter.
   */
  ...
  /**
   * @param pRepository the repository of item that should be returned by this 
   * targeter.
   */
  ...
  /**
   * <p>
   *   This method builds a full targeter/slot component path. It is built using
   *   the component path configured in Experience Manager along with the 
   *   pathPrefix property. The targeter/slot component is then resolved and the
   *   itemCount amount of items are targeted. 
   * </p>
   * <p>
   *   Items returned by slot/targeter should be of type itemDescriptor. Only 
   *   items of this type will be put in the TargetedItemsContentItem. If an
   *   item of another type is found, error will be logged. If itemCount is not
   *   set, then all items returned by slot or targeter of type itemDescriptor
   *   will be put in the TargetedItemsContentItem.
   * </p>
   * 
   * @param pCartridgeConfig - If an error occurs that is scoped to an 
    ...
    throws CartridgeHandlerException 
  {
    ...
    // If we have a targeter try and get some items from it
    if (!StringUtils.isEmpty(fullComponentPath)) {
      try {
        ...
        if (targeter != null){
          ...
          int itemCount = targetedItems.getIntProperty(ITEM_COUNT, DEFAULT_ITEMS_AMOUNT);
          if(itemCount > 0){
            NameResolver nameResolver = getSourceMap().getNameResolver(request);
            items = targeter.target(nameResolver, itemCount);
          }
          else {
          ...
          if (items != null && items.length > 0) {
            List<RepositoryItem> validItems = new ArrayList<RepositoryItem>();
            Repository exptectedRepository = getRepository();

            // Check that all items are from specified repository. As repositories
            // can contain different item types we check the items descriptor is
            // of the type we're expecting.
            for (Object item: items) {
              if (item instanceof RepositoryItem) {

            	RepositoryItem targetedItem = (RepositoryItem) item;
            	Repository itemRepository = targetedItem.getRepository();

                // Check that item is from required repository.
                if (itemRepository.equals(exptectedRepository)) {
                  
                  RepositoryItemDescriptor expectedItemDescriptor = 
                    itemRepository.getItemDescriptor(getItemDescriptor());
                  
                  // Check that item has required item descriptor and pass all 
                  // configured validators.
                  if (expectedItemDescriptor.isInstance(targetedItem)
                  && validateItem(targetedItem)) 
                  {
                    validItems.add(targetedItem);
                  }
                  else {
        ...
        AssemblerTools.getApplicationLogging().vlogError(ex, "An error occurred in TargetedItemsHandler.process");
      } 
      catch (TargetingException ex) {
        AssemblerTools.getApplicationLogging().vlogError(ex, "An error occurred in TargetedItemsHandler.process");
      }
    }
   ...
   * 
   * @return false if item fails validation, true if there should be no 
   * validation or if item is valid.
   */
